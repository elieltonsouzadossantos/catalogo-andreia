...
GitHub. Os produtos vivem num único [arquivo JSON](../public/data/produtos/produtos.json)
dentro do próprio repositório — não há banco de dados externo. A Andreia edita esse JSON
sem tocar em código, através de um painel visual (Decap CMS) que salva as mudanças
direto no GitHub por trás dos panos.
...
flowchart TD
    ...
    C -->|"webhook no push"| D["Netlify<br/>build + deploy automático"]
...
**Fonte de dados:** [`public/data/produtos/produtos.json`](../public/data/produtos/produtos.json)
é o único banco de dados do projeto. Não há Firestore, nem qualquer outro banco externo
— só esse arquivo, versionado como qualquer outro arquivo do repositório.
...
[`public/admin/config.yml`](../public/admin/config.yml) (schema do produto),
[`public/data/produtos/produtos.json`](../public/data/produtos/produtos.json) (os dados)
e [`public/js/script.js`](../public/js/script.js) (a lógica de exibição).
...
`produtos`, ligada ao arquivo
[`produtos.json`](../public/data/produtos/produtos.json). Cada produto tem os campos
abaixo — mudar essa lista é a forma de mudar o que a Andreia consegue preencher no
formulário:
